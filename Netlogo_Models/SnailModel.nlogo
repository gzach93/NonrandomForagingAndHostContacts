  ...
  ask snails [
    if not sick? AND not recovered? [
     if any? turtles-here with [sick? = true]
      [ if random 100 <= infectiousness ;if random number is less than or equal to infectionness unifected snail becomes infected
        [set sick? true
          set color black
        set sick-timer 0]
      ]
...
to move-snails ;; calls the appropriate movement mode for the snails based on the settings
  ...
  ask snails [
    ifelse stochasticity = 100 [move-snails-random]
...
<experiments>
  ...
  <experiment name="FinalSim_5Dec23" repetitions="150" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="50"/>
    <metric>resource-level</metric>
