...
to setup
  ...
  ;; The code turns
  ;; patches from lime to green as it goes, so it only counts unique neighbor combinations. Easiest to understand in the 6/6 aggregation
  ...
  ;; on different tiles to its right... 1 1 1 0. 24 total. the maximum aggregation has 24/24. the minimum aggregation has 0/24.
...
to count-all-snail-contacts ;; counts non-unique contacts between snails
  ...
  ask friends with [link-length >= 1] [ ;; break links between snails that have moved apart
    set contact-durations (lput friendship-duration contact-durations)
    die
  ]
  ask friends [ ;; adds to the duration of the contacts
    set friendship-duration (friendship-duration + 1)
  ]
  if-else ((length contact-durations + count friends) = 0) ;;; wha?
  [ set avg-contact-duration 0 ]
...
This is an agent-based model for an aquatic snail, Helisoma trivolvis, that links individual-level behavior with population-level processes, focusing on how foraging behaviors and resource distribution affect contact rates between individuals in a population.

## HOW IT WORKS

The patches were arranged in a 10 by 10 grid, and each patch had a state variable representing the biomass of the algae on the patch. The model allowed for the following algae distributions, (1) Uniform, in which all tiles were covered equally with algae, (2) Random, in which random tiles had algae on them, (3) Checkerboard, in which alternating tiles had algae on them, and (4) Center, in which algae was on all tiles in a 6 by 6 block in the center.

The model allowed for the following snail foraging behaviors, (1) Random, in which the snails moved randomly every time step, (2) Resource tracking, in which the snails moved to the neighboring cell with the most resources each timestep, (3) Optimal foraging, in which the snails stayed on a patch of algae until it dropped below a certain biomass and then found a new patch as they would with the resource tracking behavior, and (4) Variable turn radius, in which the snails had a smaller turn radius if they were not on a patch with sufficient food, and a wider turn radius if they were. This was meant to model a behavior where snails moved in a straight line until they find food and then proceed to graze in that area.

## HOW TO USE IT

The SETUP button sets up the snails and algae. The GO button starts the simulation.

The number-of-snails slider sets the initial number of snails. The movement-mode drop-down contains the above options for foraging behavior. The snail-feasting-threshold slider sets the percentage of the initial biomass of algae that serves as a threshold of resources snails find acceptable. The snail-feeding-rate is the amount of algae a snail consumes per time step.

The algae-distribution drop-down contains all of the options for resource distributions. The initial-algae-biomass determines the total amount of algae at the beginning of the experiment.
...
Philip McElmurray, Skylar R. Hopkins, Jeremy M. Wojdak, and Lisa K. Belden
@#$#@#$#@
default
true
0
...
<experiments>
  ...
  <experiment name="FinalSim_27Apr22" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="50"/>
    <metric>resource-level</metric>
